...
---
# oplan tag CLI: registry commands and --tag assignment through the daemon

Phase 5 of [[./00012-tags-registered-labels-name-colo.md]]: the CLI. Reads stay local; writes go through the daemon like every task write (`Writer` → `op_client`, `crates/op-cli/src/writer.rs:15-73`) — so `op-client` grows tag methods first. Consequence, same as all writes since the daemon became the write authority: `oplan tag create` needs a runnable daemon.
...
- Unknown-name rejections surface the server's `oplan tag create` hint and exit non-zero.

## Verify
`crates/op-cli/tests/cli.rs` on the `Project` fixture (`:29-58`): create/list/show/set/rename/delete/colors round-trip; duplicate and bad-color rejections; `create --tag` with an unknown name fails with the hint; `oplan set <id> tags "a, b"` writes a sorted set and `""` clears it; `delete` refuses referenced without `--force`.
