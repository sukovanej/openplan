...
```
(Daemon reachability / `server ping` is out of scope here — see [[./00002-run-the-server-as-a-managed-daem.md]].)
...
- **op-cli**: `create` / `get` / `show` / `set` / `delete` subcommands; a thin `reqwest`
  ...
  replacing the `ping` stub) lands in [[./00002-run-the-server-as-a-managed-daem.md]].
