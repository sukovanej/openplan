...
([[design-a-continuous-changes-accu-2380]] §7.11). The header sync control + the
...
- **`SyncStatus` schema** in `lib/api.ts` mirroring Phase 2's type
  ([[daemon-ambient-writer-accumulate-e2f2]]): `{ state, pending: TaskChange[],
...
- Primary **"Publish N to main"** action **inside** the popover -> `POST
  /api/publish` (Phase 5 [[publish-fast-forward-main-to-the-3241]]); shows
