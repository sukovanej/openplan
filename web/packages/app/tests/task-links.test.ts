...
it("links a key", () => {
  expect(split("see [[OPP-42]] first")).toEqual([
    { type: "text", value: "see " },
    { type: "link", url: "/task/OPP-42", title: null, children: [{ type: "text", value: "OPP-42" }] },
    { type: "text", value: " first" },
  ])
})
...
it("links every reference in a line", () => {
  ...
  expect(nodes?.map((n) => (n.type === "link" ? n.url : n.value))).toEqual(["/task/OPP-1", " and ", "/task/OPP-2"])
})
...
it("keeps the section suffix on a key too", () => {
  ...
  expect(nodes?.[0].type === "link" && nodes[0].url).toBe("/task/OPP-3#Design")
})

it("reads the id out of the leading digits, whatever the slug says", () => {
  const nodes = split("[[./00042-a-stale-title.md]]")
  expect(nodes?.[0].type === "link" && nodes[0].url).toBe("/task/OPP-42")
})
...
it("links nothing while the abbreviation is unknown", () => {
  expect(splitTaskRefs("see [[OPP-42]] and [[./00001-a.md]]", undefined)).toBeNull()
})
...
it("attaches as a remark plugin that rewrites the tree in place", () => {
  const [attacher, options] = taskLinkPlugins("OPP")
