...
export function Composer({
  ...
  // A session the bar reopens on mounts this disabled while its stream connects, and `autoFocus`
  // passes a disabled input by. It takes the focus once it can, unless the reader put it elsewhere.
  useEffect(() => {
    if (!disabled && document.activeElement === document.body) area.current?.focus()
  }, [disabled])
...
export function SessionState({ live }: { live: AgentSession | undefined }) {
  if (live === undefined) return <span className="text-muted-foreground text-xs">New session</span>
...
export function UsageLine({ usage }: { usage: Usage }) {
  // The cache serves the whole context again on every round trip inside a turn, so a count that
  // holds `cached_input_tokens` reads as hundreds of thousands after a few tool calls. This counts
  // the new material alone; `cost_usd` beside it is what the cache reads cost.
  const total = usage.input_tokens - usage.cached_input_tokens + usage.cache_write_tokens + usage.output_tokens
  return (
    <p className="text-muted-foreground text-xs tabular-nums">
      {total < 1000 ? total : `${Math.round(total / 1000)}k`} tokens
