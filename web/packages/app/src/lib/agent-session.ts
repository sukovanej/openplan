...
const CONNECTING: AgentSession = { phase: "connecting" }
...
export function fold(session: AgentSession, event: SessionEvent): AgentSession {
  switch (event.kind) {
    ...
    case "tasks":
      return session.phase === "live" ? { phase: "live", view: { ...session.view, tasks: event.tasks } } : session
    default: {
...
// opens with is the transcript; every later event folds into it. The state names the stream it
// folds, so another session reads as connecting from its first render.
export function useAgentSession(project: string, id: string | undefined): AgentSession | undefined {
  const stream = id === undefined ? undefined : eventsUrl(project, id)
  const [state, setState] = useState<{ readonly stream: string | undefined; readonly session: AgentSession }>({
    stream,
    session: CONNECTING,
  })
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      source.onmessage = (message: MessageEvent<string>) => {
        ...
        setState((current) => ({
          stream,
          session: fold(current.stream === stream ? current.session : CONNECTING, event),
        }))
      }
      source.onerror = () => {
        ...
        source = undefined
        if (refused) {
          setState({ stream, session: { phase: "ended" } })
          return
        }
  ...
  return state.stream === stream ? state.session : CONNECTING
}
